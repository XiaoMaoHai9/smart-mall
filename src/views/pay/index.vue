<template>
  <div class="pay">
    ...
    <div class="address">
      ...
      <div class="info" v-if="selectedAddress.address_id">
        ...
        <div class="info-address">
          {{selectedAddress.region.province}} {{selectedAddress.region.city}} {{selectedAddress.region.region}}
        </div>
      </div>
    ...
    <div class="footer-fixed">
      ...
      <div class="tipsbtn" @click="submitPay">提交订单</div>
    </div>
  </div>
</template>

<script>
import { getAddressList, submitPay } from '@/api/pay'
...
export default {
  ...
  methods: {
    // 获取订单
    async getOrder () {
      const { mode, obj } = this.$route.query
      const {
        data: {
          order,
          personal
        }
      } = await getOrder(mode, JSON.parse(obj))
      this.order = order
    ...
    async submitPay () {
      const { mode, obj } = this.$route.query
      await submitPay(mode, this.remark, JSON.parse(obj))
